...
function renderPanelCategory($pdf, $product, $conn) {
    ...
    $disc_price = floatval($product['return_amount'] ?? 0) * $quantity;
...
function renderLumberCategory($pdf, $product, $conn) {
    ...
    $dimension_id = $product['lumber_length'] ?? '';
...
$query = "SELECT * FROM product_returns WHERE id = '$id'";
$result_return = mysqli_query($conn, $query);

while($row_return = mysqli_fetch_assoc($result_return)){
    $orderid = $row_return['orderid'];
}
...
if (mysqli_num_rows($result) > 0) {
    while($row_orders = mysqli_fetch_assoc($result)){
        ...
        $query_product = "
            SELECT
                pr.pay_method as return_method,
                COALESCE(pr.discounted_price, 0) AS return_amount,
                COALESCE(pr.quantity, 0) AS return_qty,
                p.product_category,
                op.*
            FROM
                order_product AS op
            LEFT JOIN product_returns AS pr
                ON pr.orderid = op.orderid
                AND pr.productid = op.productid
        ...
        while ($row_product = mysqli_fetch_assoc($result_product)) {
            ...
            $return_amount = floatval($row_product['return_amount']);
            $amount = $return_amount * floatval($row_product['return_qty']);
            $method = $row_product['return_method'] ?? '';

            if (!isset($return_rows[$method])) {
                $return_rows[$method] = 0;
            }
            $return_rows[$method] += $amount;
        }
        ...
        foreach ($return_rows as $method => $amount) {
            $pdf->Rect($col2_x, $pdf->GetY(), $rectWidth, $rectHeight);

            $pdf->SetXY($col2_x, $pdf->GetY());
            $pdf->SetFont('Arial', 'B', 9);
            $pdf->Cell(30, $lineheight, 'Payment Type:', 0, 0, 'R');
            $pdf->SetFont('Arial', '', 9);
            $pdf->Cell(35, $lineheight, ucwords($method), 0, 1, 'L');

            $pdf->SetXY($col2_x, $pdf->GetY());
            $pdf->SetFont('Arial', 'B', 9);
            $pdf->Cell(30, $lineheight, 'Amount:', 0, 0, 'R');
            $pdf->SetFont('Arial', '', 9);
            $pdf->Cell(35, $lineheight, '$' . number_format($amount, 2), 0, 1, 'L');
        ...
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Cell($rectWidth, $lineheight, 'PRODUCT RETURN', 1, 1, 'C', true);
