    ...
    <div class="col-3">
        <select class="form-control qty_select2" id="qty-gauge" name="gauge">
            ...
            <optgroup label="Product Gauges">
                <?php
                $query_gauge = "SELECT * FROM product_gauge WHERE hidden = '0' AND status = '1' AND product_gauge_id IN (1,2) ORDER BY product_gauge ASC";
...
$(function() {
    ...
    function shouldCalculateBackerRod() {
        console.log(123);
        return $("#qty-gauge").val() == "26 GAUGE";
    }

    function handleBackerRod() {
        if (shouldCalculateBackerRod()) {
            calculateBackerRod();
        } else {
    ...
    $(document).off("change", "#qty-gauge").on("change", "#qty-gauge", handleBackerRod);

    $(document).off("change", ".panel_style").on("change", ".panel_style", handleBackerRod);

    $(document).off("input", ".quantity-product, .length_feet, .length_inch")
        .on("input", ".quantity-product, .length_feet, .length_inch", handleBackerRod);
});
</script>