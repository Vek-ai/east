class ChatWidget {
    ...
    async loadMessages(staffId) {
        ...
        try {
            const response = await fetch(`api/get_messages.php?other_staff_id=${staffId}&last_message_id=${chat.lastMessageId}`);
            const data = await response.json();
    ...
    displayMessage(staffId, message) {
        ...
        const fileHtml = message.file_name ? `
            <div class="chat-message-file" onclick="window.open('uploads/${message.file_path}', '_blank')">
