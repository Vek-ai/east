...
$table = 'product_screw_coating';
$test_table = 'product_screw_coating_excel';

$includedColumns = [
    'product_screw_coating_id'   => 'ID',
    'product_screw_coating'      => 'Screw Type',
...
if(isset($_REQUEST['action'])) {
    ...
    if ($action == "add_update") {
        $product_screw_coating_id = mysqli_real_escape_string($conn, $_POST['product_screw_coating_id']);
        $product_screw_coating = mysqli_real_escape_string($conn, $_POST['product_screw_coating']);
        $abbreviation = mysqli_real_escape_string($conn, $_POST['abbreviation']);
        $notes = mysqli_real_escape_string($conn, $_POST['notes']);
        $userid = mysqli_real_escape_string($conn, $_POST['userid']);

        $checkQuery = "SELECT abbreviation FROM product_screw_coating WHERE product_screw_coating_id = '$product_screw_coating_id'";
        $result = mysqli_query($conn, $checkQuery);

        if (mysqli_num_rows($result) > 0) {
            $row = mysqli_fetch_assoc($result);
            $old_abbr = $row['abbreviation'];

            $updateQuery = "UPDATE product_screw_coating 
                            SET product_screw_coating = '$product_screw_coating',
                                abbreviation = '$abbreviation',
                                notes = '$notes',
                                last_edit = NOW(),
                                edited_by = '$userid'
                            WHERE product_screw_coating_id = '$product_screw_coating_id'";

            if (mysqli_query($conn, $updateQuery)) {
                echo "update-success";
                if ($old_abbr !== $abbreviation) {
                    regenerateABR('product_screw_coating', $product_screw_coating_id);
                }
            } else {
                echo "Error updating product type: " . mysqli_error($conn);
            }
        } else {
            $insertQuery = "INSERT INTO product_screw_coating 
                            (product_screw_coating, abbreviation, notes, added_date, added_by) 
                            VALUES 
                            ('$product_screw_coating', '$abbreviation', '$notes', NOW(), '$userid')";
    ...
    if ($action == "change_status") {
        ...
        $statusQuery = "UPDATE product_screw_coating SET status = '$new_status' WHERE product_screw_coating_id = '$product_screw_coating_id'";
    ...
    if ($action == 'hide_product_screw_coating') {
        $product_screw_coating_id = mysqli_real_escape_string($conn, $_POST['product_screw_coating_id']);
        $query = "UPDATE product_screw_coating SET hidden='1' WHERE product_screw_coating_id='$product_screw_coating_id'";
    ...
    if ($action == 'fetch_modal_content') {
        $product_screw_coating_id = mysqli_real_escape_string($conn, $_POST['id']);
        ...
        ?>
            <div class="row pt-3">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" id="product_screw_coating" name="product_screw_coating" class="form-control"  value="<?= $row['product_screw_coating'] ?? '' ?>"/>
                    </div>
                </div>
            ...
            <input type="hidden" id="product_screw_coating_id" name="product_screw_coating_id" class="form-control"  value="<?= $product_screw_coating_id ?>"/>
        <?php
    }
    ...
    if ($action === 'fetch_table') {
        ...
        $query = "SELECT * FROM product_screw_coating WHERE hidden = 0";
        $result = mysqli_query($conn, $query);
    
        $data = [];
        while ($row = mysqli_fetch_assoc($result)) {
            $no = $row['product_screw_coating_id'];
            $product_screw_coating = $row['product_screw_coating'];
            ...
            $data[] = [
                'product_screw_coating' => $product_screw_coating,
                'abbreviation' => $abbreviation,
                'notes' => $notes,
