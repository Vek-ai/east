...
if(!empty($_REQUEST['truss_ceiling_load_id'])){
  $truss_ceiling_load_id = $_REQUEST['truss_ceiling_load_id'];
  $query = "SELECT * FROM truss_ceiling_load WHERE truss_ceiling_load_id = '$truss_ceiling_load_id'";
  $result = mysqli_query($conn, $query);            
  while ($row = mysqli_fetch_array($result)) {
      $truss_ceiling_load_id = $row['truss_ceiling_load_id'];
      $truss_ceiling_load = $row['truss_ceiling_load'];
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="table-responsive">
                <table id="display_truss_ceiling_load" class="table table-striped table-bordered text-nowrap align-middle">
                  <thead>
                    <tr>
                      <th><?= $page_title ?></th>
                      <th>Description</th>
                  ...
                  <?php
                  $no = 1;
                  $query_truss_ceiling_load = "SELECT * FROM truss_ceiling_load WHERE hidden = 0";
                  $result_truss_ceiling_load = mysqli_query($conn, $query_truss_ceiling_load);            
                  while ($row_truss_ceiling_load = mysqli_fetch_array($result_truss_ceiling_load)) {
                      $truss_ceiling_load_id = $row_truss_ceiling_load['truss_ceiling_load_id'];
                      $truss_ceiling_load = $row_truss_ceiling_load['truss_ceiling_load'];
                      $description = $row_truss_ceiling_load['description'];
                      $db_status = $row_truss_ceiling_load['status'];

                      $date = new DateTime($row_truss_ceiling_load['last_edit']);
                      $last_edit = $date->format('m-d-Y');

                      $added_by = $row_truss_ceiling_load['added_by'];
                      $edited_by = $row_truss_ceiling_load['edited_by'];
                      ...
                      if ($row_truss_ceiling_load['status'] == '0') {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$truss_ceiling_load_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-danger bg-danger text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Inactive</div></a>";
                      } else {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$truss_ceiling_load_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Active</div></a>";
                      }
                  ?>
                  <tr id="product-row-<?= $no ?>">
                      <td><span class="product<?= $no ?> <?php if ($row_truss_ceiling_load['status'] == '0') { echo 'emphasize-strike'; } ?>"><?= $truss_ceiling_load ?></span></td>
                      ...
                      <td class="text-center" id="action-button-<?= $no ?>">
                        ...
                        ?>
                          ...
                          <?php if ($row_truss_ceiling_load['status'] == '0') { ?>
                                <a href="#" title="Archive" class="py-1 text-dark hideTrussCeilingLoad" data-id="<?= $truss_ceiling_load_id ?>" data-row="<?= $no ?>">
                                    <i class="text-danger ti ti-trash fs-7"></i>
                                </a>
                          <?php } else { ?>
                                <a href="#" title="Edit" id="addModalBtn" class="d-flex align-items-center justify-content-center text-decoration-none" data-id="<?= $truss_ceiling_load_id ?>" data-type="edit">
                  ...
                  $(document).ready(function() {
                      ...
                      $(document).on('click', '.changeStatus', function(event) {
                          ...
                          var truss_ceiling_load_id = $(this).data('id');
                          var status = $(this).data('status');
                          var no = $(this).data('no');
                          $.ajax({
                              url: 'pages/truss_ceiling_load_ajax.php',
                              type: 'POST',
                              data: {
                                  truss_ceiling_load_id: truss_ceiling_load_id,
                                  status: status,
                              ...
                              success: function(response) {
                                  if (response == 'success') {
                                      if (status == 1) {
                                          ...
                                          $('#action-button-' + no).html('<a href="#" title="Archive" class="py-1 text-dark hideTrussCeilingLoad" data-id="' + truss_ceiling_load_id + '" data-row="' + no + '"><i class="text-danger ti ti-trash fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        } else {
                                          $('#status-alert' + no).removeClass().addClass('alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0').text('Active');
                                          $(".changeStatus[data-no='" + no + "']").data('status', "1");
                                          $('.product' + no).removeClass('emphasize-strike'); // Remove emphasize-strike class
                                          $('#action-button-' + no).html('<a href="#" title="Edit" id="addModalBtn" data-id="' + truss_ceiling_load_id + '" data-type="edit" class="py-1"><i class="ti ti-pencil fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        }
                                  } else {
                      ...
                      $(document).on('click', '.hideTrussCeilingLoad', function(event) {
                          event.preventDefault();
                          var truss_ceiling_load_id = $(this).data('id');
                          ...
                          $.ajax({
                              ...
                              data: {
                                  truss_ceiling_load_id: truss_ceiling_load_id,
                                  action: 'hide_truss_ceiling_load'
                              },
                              success: function(response) {
                                  if (response == 'success') {
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form id="trussCeilingLoadForm" class="form-horizontal">
                <div class="modal-body">
                    <div class="card">
                        <div class="card-body">
...
<script>
  $(document).ready(function() {
    ...
    var table = $('#display_truss_ceiling_load').DataTable({
        pageLength: 100
    });

    $('#display_truss_ceiling_load_filter').hide();
    ...
    $('#trussCeilingLoadForm').on('submit', function(event) {
        event.preventDefault(); 
            ...
            success: function(response) {
              ...
              if (response.trim() === "success_update") {
                  ...
                  $('#response-modal').on('hide.bs.modal', function () {
                    window.location.href = "?page=truss_ceiling_load";
                  });
    ...
    $(document).on('click', '#downloadBtn', function(event) {
        window.location.href = "pages/truss_ceiling_load_ajax.php?action=download_excel";
    });
