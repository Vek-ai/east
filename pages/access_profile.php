          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="table-responsive">
                <table id="display_access_profile" class="table table-striped table-bordered text-nowrap align-middle">
                  <thead>
                    <tr>
                      <th><?= $page_title ?></th>
                      <th>Description</th>
                  ...
                  <?php
                  $no = 1;
                  $query_access_profile = "SELECT * FROM access_profile WHERE hidden = 0";
                  $result_access_profile = mysqli_query($conn, $query_access_profile);            
                  while ($row_access_profile = mysqli_fetch_array($result_access_profile)) {
                      $access_profile_id = $row_access_profile['access_profile_id'];
                      $access_profile = $row_access_profile['access_profile'];
                      $access_profile = $row_access_profile['access_profile'];
                      $description = $row_access_profile['description'];
                      $db_status = $row_access_profile['status'];

                      $date = new DateTime($row_access_profile['last_edit']);
                      $last_edit = $date->format('m-d-Y');

                      $added_by = $row_access_profile['added_by'];
                      $edited_by = $row_access_profile['edited_by'];
                      ...
                      if ($row_access_profile['status'] == '0') {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$access_profile_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-danger bg-danger text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Inactive</div></a>";
                      } else {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$access_profile_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Active</div></a>";
                      }
                  ?>
                  <tr id="product-row-<?= $no ?>">
                      <td><span class="product<?= $no ?> <?php if ($row_access_profile['status'] == '0') { echo 'emphasize-strike'; } ?>"><?= $access_profile ?></span></td>
                      ...
                      <td class="text-center" id="action-button-<?= $no ?>">
                        ...
                        if ($permission === 'edit') {
                        ?>
                          <?php if ($row_access_profile['status'] == '0') { ?>
                                <a href="#" class="py-1 text-dark hideAccessProfile" title="Archive" data-id="<?= $access_profile_id ?>" data-row="<?= $no ?>">
                                    <i class="text-danger ti ti-trash fs-7"></i>
                                </a>
                          <?php } else { ?>
                                <a href="#" id="addModalBtn" title="Edit" class="d-flex align-items-center justify-content-center text-decoration-none" data-id="<?= $access_profile_id ?>" data-type="edit">
                  ...
                  $(document).ready(function() {
                      ...
                      $(document).on('click', '.changeStatus', function(event) {
                          ...
                          var access_profile_id = $(this).data('id');
                          var status = $(this).data('status');
                          var no = $(this).data('no');
                          $.ajax({
                              url: 'pages/access_profile_ajax.php',
                              type: 'POST',
                              data: {
                              ...
                              success: function(response) {
                                  if (response == 'success') {
                                      if (status == 1) {
                                          ...
                                          $('#action-button-' + no).html('<a href="#" title="Archive" class="py-1 text-dark hideAccessProfile" data-id="' + access_profile_id + '" data-row="' + no + '"><i class="text-danger ti ti-trash fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        } else {
                                          $('#status-alert' + no).removeClass().addClass('alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0').text('Active');
                                          $(".changeStatus[data-no='" + no + "']").data('status', "1");
                                          $('.product' + no).removeClass('emphasize-strike'); // Remove emphasize-strike class
                                          $('#action-button-' + no).html('<a href="#" title="Edit" id="addModalBtn" data-id="' + access_profile_id + '" data-type="edit" class="py-1"><i class="ti ti-pencil fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        }
                                  } else {
                      ...
                      $(document).on('click', '.hideAccessProfile', function(event) {
                          event.preventDefault();
                          var access_profile_id = $(this).data('id');
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form id="accessProfileForm" class="form-horizontal">
                <div class="modal-body">
                    <div class="card">
                        <div class="card-body">
...
<script>
  $(document).ready(function() {
    ...
    var table = $('#display_access_profile').DataTable({
        pageLength: 100
    });

    $('#display_access_profile_filter').hide();
    ...
    $('#accessProfileForm').on('submit', function(event) {
        event.preventDefault(); 
            ...
            success: function(response) {
              ...
              if (response.trim() === "success_update") {
                  ...
                  $('#response-modal').on('hide.bs.modal', function () {
                    window.location.href = "?page=access_profile";
                  });
    ...
    $(document).on('click', '#downloadBtn', function(event) {
        window.location.href = "pages/access_profile_ajax.php?action=download_excel";
    });
