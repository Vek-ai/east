              ...
              <div class="table-responsive">
                <table id="display_roll_former" class="table table-striped table-bordered align-middle">
                  ...
                  while ($row_roll_former = mysqli_fetch_array($result_roll_former)) {
                      ...
                      if ($row_roll_former['status'] == '0') {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$roll_former_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-danger bg-danger text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Inactive</div></a>";
                      } else {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$roll_former_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Active</div></a>";
                      }
                  ?>
                  <tr id="product-row-<?= $no ?>">
                      ...
                      <td class="text-center" id="action-button-<?= $no ?>">
                          <?php if ($row_roll_former['status'] == '0') { ?>
                                <a href="#" class="py-1 text-dark hideSupplierType" title="Archive" data-id="<?= $roll_former_id ?>" data-row="<?= $no ?>">
                                    <i class="text-danger ti ti-trash fs-7"></i>
                                </a>
                          <?php } else { ?>
                                <a href="#" id="addModalBtn" title="Edit" class="d-flex align-items-center justify-content-center text-decoration-none" data-id="<?= $roll_former_id ?>" data-type="edit">
                  ...
                  $(document).ready(function() {
                      ...
                      $(document).on('click', '.changeStatus', function(event) {
                          ...
                          var roll_former_id = $(this).data('id');
                          var status = $(this).data('status');
                          var no = $(this).data('no');
                          $.ajax({
                              url: 'pages/roll_former_ajax.php',
                              type: 'POST',
                              data: {
                              ...
                              success: function(response) {
                                  if (response == 'success') {
                                      if (status == 1) {
                                          ...
                                          $('#action-button-' + no).html('<a href="#" class="py-1 text-dark hideSupplierType" title="Archive" data-id="' + roll_former_id + '" data-row="' + no + '"><i class="text-danger ti ti-trash fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        } else {
                                          $('#status-alert' + no).removeClass().addClass('alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0').text('Active');
                                          $(".changeStatus[data-no='" + no + "']").data('status', "1");
                                          $('.product' + no).removeClass('emphasize-strike'); // Remove emphasize-strike class
                                          $('#action-button-' + no).html('<a href="#" id="addModalBtn" title="Edit" data-id="' + roll_former_id + '" data-type="edit" class="py-1"><i class="ti ti-pencil fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        }
                                  } else {
...
<script>
  $(document).ready(function() {
    ...
    var table = $('#display_roll_former').DataTable({
        pageLength: 100
    });

    $('#display_roll_former_filter').hide();
            ...
            success: function(response) {
              ...
              if (response.trim() == "success_update") {
                  ...
                  $('#response-modal').on('hide.bs.modal', function () {
                    window.location.href = "?page=roll_former";
                  });
    ...
    $(document).on('click', '#downloadBtn', function(event) {
        window.location.href = "pages/roll_former_ajax.php?action=download_excel";
    });
