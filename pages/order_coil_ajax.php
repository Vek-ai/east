...
if (isset($_REQUEST['query'])) {
    ...
    if (!empty($searchQuery)) {
        $query_coil .= " AND (coil LIKE '%$searchQuery%')";
    }

    if (!empty($color_id)) {
        $query_coil .= " AND color = '$color_id'";
    }
    ...
    if (mysqli_num_rows($result_coil) > 0) {
        while ($row_coil = mysqli_fetch_array($result_coil)) {
            ...
            $product_length = $row_coil['length'];
            $product_width = $row_coil['width'];
            $product_color = $row_coil['color'];
            ...
            $quantity = 0;
            //logic for coil quantity available later, default 0 for now

            if ($quantity > 0) {
                $stock_text = '
                    <a href="javascript:void(0);" id="view_in_stock" data-id="' . $row_coil['product_id'] . '" class="d-flex align-items-center">
                        <span class="text-bg-success p-1 rounded-circle"></span>
                        <span class="ms-2">In Stock</span>
                    </a>';
            } else {
                $stock_text = '
                    <a href="javascript:void(0);" id="view_out_of_stock" data-id="' . $row_coil['product_id'] . '" class="d-flex align-items-center">
                        <span class="text-bg-danger p-1 rounded-circle"></span>
                        <span class="ms-2">Out of Stock</span>
                    </a>';
            }
            ...
            <tr>
                <td style="width: 15% !important;">
                    <a href="javascript:void(0);" id="view_coil_details" data-id="' . $row_coil['coil_id'] . '" class="d-flex align-items-center text-left">
                        <div class="d-flex align-items-center">
                            <img src="'.$picture_path.'" class="rounded-circle" alt="materialpro-img" width="56" height="56">
                            <div class="ms-3">
                                <h6 class="fw-semibold mb-0 fs-4">'. $row_coil['coil'] .' ' .$dimensions .'</h6>
                            </div>
                        </div>
                    </a>
                </td>
                <td>
                    <div class="d-flex mb-0 gap-8">
                        <a class="rounded-circle d-block p-6" href="javascript:void(0)" style="background-color:' .getColorHexFromColorID($row_coil['color']) .'" data-toggle="tooltip" data-placement="top" title="'.getColorName($row_coil['color']).'"></a> '
                         .'
                    </div>
                </td>
                <td><p class="mb-0">'. getProductTypeName($row_coil['type']) .'</p></td>
                <td><p class="mb-0">'. getProductLineName($row_coil['line']) .'</p></td>
                <td><p class="mb-0">'. getProductCategoryName($row_coil['category']) .'</p></td>
                <td>
                    <div class="d-flex align-items-center text-center">'.$stock_text.'</div>
                </td>
                <td><h6 class="mb-0 fs-4">'. 'N/A' .'</h6></td>
                <td>
                    <button class="btn btn-primary btn-add-to-cart px-2 py-1" type="button" data-id="'.$row_coil['coil_id'].'" onClick="addToOrdersCoil(this)">Add to Order</button>
                </td>
            </tr>';
        }
    }
