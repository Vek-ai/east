...
if(!empty($_REQUEST['truss_overhang_id'])){
  $truss_overhang_id = $_REQUEST['truss_overhang_id'];
  $query = "SELECT * FROM truss_overhang WHERE truss_overhang_id = '$truss_overhang_id'";
  $result = mysqli_query($conn, $query);            
  while ($row = mysqli_fetch_array($result)) {
      $truss_overhang_id = $row['truss_overhang_id'];
      $truss_overhang = $row['truss_overhang'];
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="table-responsive">
                <table id="display_truss_overhang" class="table table-striped table-bordered text-nowrap align-middle">
                  <thead>
                    <tr>
                      <th><?= $page_title ?></th>
                      <th>Description</th>
                  ...
                  <?php
                  $no = 1;
                  $query_truss_overhang = "SELECT * FROM truss_overhang WHERE hidden = 0";
                  $result_truss_overhang = mysqli_query($conn, $query_truss_overhang);            
                  while ($row_truss_overhang = mysqli_fetch_array($result_truss_overhang)) {
                      $truss_overhang_id = $row_truss_overhang['truss_overhang_id'];
                      $truss_overhang = $row_truss_overhang['truss_overhang'];
                      $unit_of_measure = $row_truss_overhang['unit_of_measure'];
                      $truss_overhang = $row_truss_overhang['truss_overhang'];
                      $description = $row_truss_overhang['description'];
                      $db_status = $row_truss_overhang['status'];

                      $date = new DateTime($row_truss_overhang['last_edit']);
                      $last_edit = $date->format('m-d-Y');

                      $added_by = $row_truss_overhang['added_by'];
                      $edited_by = $row_truss_overhang['edited_by'];
                      ...
                      if ($row_truss_overhang['status'] == '0') {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$truss_overhang_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-danger bg-danger text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Inactive</div></a>";
                      } else {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$truss_overhang_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Active</div></a>";
                      }
                  ?>
                  <tr id="product-row-<?= $no ?>">
                      <td><span class="product<?= $no ?> <?php if ($row_truss_overhang['status'] == '0') { echo 'emphasize-strike'; } ?>"><?= $truss_overhang .' ' .$unit_of_measure ?></span></td>
                      ...
                      <td class="text-center" id="action-button-<?= $no ?>">
                        ...
                        if ($permission === 'edit') {
                        ?>
                          <?php if ($row_truss_overhang['status'] == '0') { ?>
                                <a href="#" class="py-1 text-dark hideTrussOverhang" title="Archive" data-id="<?= $truss_overhang_id ?>" data-row="<?= $no ?>">
                                    <i class="text-danger ti ti-trash fs-7"></i>
                                </a>
                          <?php } else { ?>
                                <a href="#" id="addModalBtn" title="Edit" class="d-flex align-items-center justify-content-center text-decoration-none" data-id="<?= $truss_overhang_id ?>" data-type="edit">
                  ...
                  $(document).ready(function() {
                      ...
                      $(document).on('click', '.changeStatus', function(event) {
                          ...
                          var truss_overhang_id = $(this).data('id');
                          var status = $(this).data('status');
                          var no = $(this).data('no');
                          $.ajax({
                              url: 'pages/truss_overhang_ajax.php',
                              type: 'POST',
                              data: {
                                  truss_overhang_id: truss_overhang_id,
                                  status: status,
                              ...
                              success: function(response) {
                                  if (response == 'success') {
                                      if (status == 1) {
                                          ...
                                          $('#action-button-' + no).html('<a href="#" title="Archive" class="py-1 text-dark hideTrussOverhang" data-id="' + truss_overhang_id + '" data-row="' + no + '"><i class="text-danger ti ti-trash fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        } else {
                                          $('#status-alert' + no).removeClass().addClass('alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0').text('Active');
                                          $(".changeStatus[data-no='" + no + "']").data('status', "1");
                                          $('.product' + no).removeClass('emphasize-strike'); // Remove emphasize-strike class
                                          $('#action-button-' + no).html('<a href="#" title="Edit" id="addModalBtn" data-id="' + truss_overhang_id + '" data-type="edit" class="py-1"><i class="ti ti-pencil fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        }
                                  } else {
                      ...
                      $(document).on('click', '.hideTrussOverhang', function(event) {
                          event.preventDefault();
                          var truss_overhang_id = $(this).data('id');
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form id="trussOverhangForm" class="form-horizontal">
                <div class="modal-body">
                    <div class="card">
                        <div class="card-body">
...
<script>
  $(document).ready(function() {
    ...
    var table = $('#display_truss_overhang').DataTable({
        pageLength: 100
    });

    $('#display_truss_overhang_filter').hide();
    ...
    $('#trussOverhangForm').on('submit', function(event) {
        event.preventDefault(); 
            ...
            success: function(response) {
              ...
              if (response.trim() === "success_update") {
                  ...
                  $('#response-modal').on('hide.bs.modal', function () {
                    window.location.href = "?page=truss_overhang";
                  });
    ...
    $(document).on('click', '#downloadBtn', function(event) {
        window.location.href = "pages/truss_overhang_ajax.php?action=download_excel";
    });
