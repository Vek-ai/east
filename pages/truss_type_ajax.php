...
if(isset($_REQUEST['action'])) {
    ...
    if ($action == "add_update") {
        $truss_type_id = mysqli_real_escape_string($conn, $_POST['truss_type_id']);
        ...
        $checkQuery = "SELECT * FROM truss_type WHERE truss_type_id = '$truss_type_id'";
        $result = mysqli_query($conn, $checkQuery);

        if (mysqli_num_rows($result) > 0) {
            // Record exists, fetch current values
            $row = mysqli_fetch_assoc($result);
            $current_truss_type = $row['truss_type'];

            $duplicates = array();

            // Check for duplicates only if the new values are different from the current values
            if ($truss_type != $current_truss_type) {
                $checkCategory = "SELECT * FROM truss_type WHERE truss_type = '$current_truss_type'";
                $resultCategory = mysqli_query($conn, $checkCategory);
                if (mysqli_num_rows($resultCategory) > 0) {
                    $duplicates[] = "Truss Type";
                }
            }
            ...
            if (!empty($duplicates)) {
                ...
                $updateQuery = "UPDATE truss_type SET truss_type = '$truss_type', description = '$description', last_edit = NOW(), edited_by = '$userid'  WHERE truss_type_id = '$truss_type_id'";
            ...
            $duplicates = array();
            $checkCategory = "SELECT * FROM truss_type WHERE truss_type = '$truss_type'";
            $resultCategory = mysqli_query($conn, $checkCategory);
            if (mysqli_num_rows($resultCategory) > 0) {
                $duplicates[] = "Truss Type";
            }
            ...
            if(!empty($duplicates)){
                ...
                $insertQuery = "INSERT INTO truss_type (truss_type, description, added_date, added_by) VALUES ('$truss_type', '$description', NOW(), '$userid')";
    ...
    if ($action == "change_status") {
        ...
        $statusQuery = "UPDATE truss_type SET status = '$new_status' WHERE truss_type_id = '$truss_type_id'";
    ...
    if ($action == 'hide_truss_type') {
        $truss_type_id = mysqli_real_escape_string($conn, $_POST['truss_type_id']);
        $query = "UPDATE truss_type SET hidden='1' WHERE truss_type_id='$truss_type_id'";
        ...
        ?>
            <div class="row pt-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="truss_type" name="truss_type" class="form-control"  value="<?= $truss_type ?>"/>
                </div>
                </div>
            </div>
            ...
            <input type="hidden" id="truss_type_id" name="truss_type_id" class="form-control"  value="<?= $id ?>"/>
        <?php
    }
