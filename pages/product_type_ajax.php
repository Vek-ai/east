...
if(isset($_REQUEST['action'])) {
    ...
    if ($action == "add_update") {
        ...
        $multiplier = floatval(mysqli_real_escape_string($conn, $_POST['multiplier'] ?? 0.00));
        $special = isset($_POST['special']) ? intval($_POST['special']) : 0;
        $userid = mysqli_real_escape_string($conn, $_POST['userid']);

        $checkQuery = "SELECT type_abreviations FROM product_type WHERE product_type_id = '$product_type_id'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $updateQuery = "UPDATE product_type 
            ...
            if (mysqli_query($conn, $updateQuery)) {
                ...
                if ($old_abbr !== $type_abreviations) {
                    regenerateABR('product_type', $product_type_id);
                }
            } else {
            ...
            $insertQuery = "INSERT INTO product_type 
                            (product_type, type_abreviations, product_category, notes, multiplier, special, added_date, added_by) 
                            VALUES 
                            ('$product_type', '$type_abreviations', '$product_category', '$notes', '$multiplier', '$special', NOW(), '$userid')";
    ...
    if ($action === 'fetch_table') {
        ...
        $query = "SELECT * FROM product_type WHERE hidden = 0";
        $result = mysqli_query($conn, $query);
    
        $data = [];
        while ($row = mysqli_fetch_assoc($result)) {
            $no = $row['product_type_id'];
