...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.tag-rework-btn', function (e) {
            ...
            if (!confirm('Tag this coil as Transferred to Inventory?')) return;

            $.ajax({
                url: 'pages/coils_rework_done_ajax.php',
                type: 'POST',
                data: {
                    action: 'coil_tag_release',
                    coil_defective_id: coil_defective_id
                },
                success: function (response) {
                    if (response.trim() === 'success') {
                        alert('Coil tagged as Transferred to Inventory.');
                        location.reload();
                    } else {
