...
$table = 'truss_spacing';
$test_table = 'truss_spacing_excel';
...
if(isset($_REQUEST['action'])) {
    ...
    if ($action == "add_update") {
        $truss_spacing_id = mysqli_real_escape_string($conn, $_POST['truss_spacing_id']);
        ...
        $checkQuery = "SELECT * FROM truss_spacing WHERE truss_spacing_id = '$truss_spacing_id'";
        $result = mysqli_query($conn, $checkQuery);

        if (mysqli_num_rows($result) > 0) {
            // No duplicates, proceed with update
            $updateQuery = "UPDATE truss_spacing SET truss_spacing = '$truss_spacing', unit_of_measure = '$unit_of_measure', description = '$description', last_edit = NOW(), edited_by = '$userid'  WHERE truss_spacing_id = '$truss_spacing_id'";
            if (mysqli_query($conn, $updateQuery)) {
                echo "success_update";
            } else {
                echo "Error updating Truss Spacing: " . mysqli_error($conn);
            }
        } else {
            // Record does not exist, perform duplicate checks before inserting
            $duplicates = array();
            $checkCategory = "SELECT * FROM truss_spacing WHERE truss_spacing = '$truss_spacing' AND unit_of_measure = '$unit_of_measure'";
            ...
            if(!empty($duplicates)){
                ...
                $insertQuery = "INSERT INTO truss_spacing (truss_spacing, unit_of_measure, description, added_date, added_by) VALUES ('$truss_spacing', '$unit_of_measure', '$description', NOW(), '$userid')";
                if (mysqli_query($conn, $insertQuery)) {
                    echo "sucess_add";
                } else {
                    echo "Error adding Truss Spacing: " . mysqli_error($conn);
                }
            }
        }
    } 
    ...
    if ($action == "change_status") {
        ...
        $statusQuery = "UPDATE truss_spacing SET status = '$new_status' WHERE truss_spacing_id = '$truss_spacing_id'";
    ...
    if ($action == 'hide_truss_spacing') {
        $truss_spacing_id = mysqli_real_escape_string($conn, $_POST['truss_spacing_id']);
        $query = "UPDATE truss_spacing SET hidden='1' WHERE truss_spacing_id='$truss_spacing_id'";
    ...
    if ($action == 'fetch_modal_content') {
        ...
        ?>
            <div class="row pt-3">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label class="form-label">Truss Spacing Measurement</label>
                        <input type="number" id="truss_spacing" name="truss_spacing" class="form-control"  value="<?= $truss_spacing ?>"/>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
            ...
            <input type="hidden" id="truss_spacing_id" name="truss_spacing_id" class="form-control"  value="<?= $id ?>"/>
        <?php
    }
