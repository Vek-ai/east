...
$table = 'truss_overhang';
$test_table = 'truss_overhang_excel';
...
if(isset($_REQUEST['action'])) {
    ...
    if ($action == "add_update") {
        $truss_overhang_id = mysqli_real_escape_string($conn, $_POST['truss_overhang_id']);
        $truss_overhang = mysqli_real_escape_string($conn, $_POST['truss_overhang']);
        ...
        $checkQuery = "SELECT * FROM truss_overhang WHERE truss_overhang_id = '$truss_overhang_id'";
        $result = mysqli_query($conn, $checkQuery);

        if (mysqli_num_rows($result) > 0) {
            // No duplicates, proceed with update
            $updateQuery = "UPDATE truss_overhang SET truss_overhang = '$truss_overhang', unit_of_measure = '$unit_of_measure', description = '$description', last_edit = NOW(), edited_by = '$userid'  WHERE truss_overhang_id = '$truss_overhang_id'";
            if (mysqli_query($conn, $updateQuery)) {
                echo "success_update";
            } else {
                echo "Error updating Truss Overhang: " . mysqli_error($conn);
            }
        } else {
            // Record does not exist, perform duplicate checks before inserting
            $duplicates = array();
            $checkCategory = "SELECT * FROM truss_overhang WHERE truss_overhang = '$truss_overhang' AND unit_of_measure = '$unit_of_measure'";
            ...
            if(!empty($duplicates)){
                ...
                $insertQuery = "INSERT INTO truss_overhang (truss_overhang, unit_of_measure, description, added_date, added_by) VALUES ('$truss_overhang', '$unit_of_measure', '$description', NOW(), '$userid')";
                if (mysqli_query($conn, $insertQuery)) {
                    echo "sucess_add";
                } else {
                    echo "Error adding Truss Overhang: " . mysqli_error($conn);
                }
            }
        }
    } 
    ...
    if ($action == "change_status") {
        ...
        $statusQuery = "UPDATE truss_overhang SET status = '$new_status' WHERE truss_overhang_id = '$truss_overhang_id'";
    ...
    if ($action == 'hide_truss_overhang') {
        $truss_overhang_id = mysqli_real_escape_string($conn, $_POST['truss_overhang_id']);
        $query = "UPDATE truss_overhang SET hidden='1' WHERE truss_overhang_id='$truss_overhang_id'";
    ...
    if ($action == 'fetch_modal_content') {
        ...
        ?>
            <div class="row pt-3">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label class="form-label">Truss Overhang Measurement</label>
                        <input type="number" id="truss_overhang" name="truss_overhang" class="form-control"  value="<?= $truss_overhang ?>"/>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="unitOfMeasure" name="unit_of_measure" required>
                            <option value="" disabled <?= empty($unit_of_measure) ? 'selected' : '' ?>>Select unit</option>
                            <option value="ft" <?= ($unit_of_measure === 'ft') ? 'selected' : '' ?>>Feet (ft)</option>
                            <option value="in" <?= ($unit_of_measure === 'in') ? 'selected' : '' ?>>Inches (in)</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <input type="hidden" id="truss_overhang_id" name="truss_overhang_id" class="form-control"  value="<?= $id ?>"/>
        <?php
    }
