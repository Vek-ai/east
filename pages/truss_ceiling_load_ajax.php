...
$table = 'truss_ceiling_load';
$test_table = 'truss_ceiling_load_excel';
...
if(isset($_REQUEST['action'])) {
    ...
    if ($action == "add_update") {
        $truss_ceiling_load_id = mysqli_real_escape_string($conn, $_POST['truss_ceiling_load_id']);
        ...
        $checkQuery = "SELECT * FROM truss_ceiling_load WHERE truss_ceiling_load_id = '$truss_ceiling_load_id'";
        $result = mysqli_query($conn, $checkQuery);

        if (mysqli_num_rows($result) > 0) {
            // Record exists, fetch current values
            $row = mysqli_fetch_assoc($result);
            $current_truss_ceiling_load = $row['truss_ceiling_load'];

            $duplicates = array();

            // Check for duplicates only if the new values are different from the current values
            if ($truss_ceiling_load != $current_truss_ceiling_load) {
                $checkCategory = "SELECT * FROM truss_ceiling_load WHERE truss_ceiling_load = '$current_truss_ceiling_load'";
                $resultCategory = mysqli_query($conn, $checkCategory);
                if (mysqli_num_rows($resultCategory) > 0) {
                    $duplicates[] = "Truss Ceiling Load";
                }
            }
            ...
            if (!empty($duplicates)) {
                ...
                $updateQuery = "UPDATE truss_ceiling_load SET truss_ceiling_load = '$truss_ceiling_load', description = '$description', last_edit = NOW(), edited_by = '$userid'  WHERE truss_ceiling_load_id = '$truss_ceiling_load_id'";
            ...
            $duplicates = array();
            $checkCategory = "SELECT * FROM truss_ceiling_load WHERE truss_ceiling_load = '$truss_ceiling_load'";
            $resultCategory = mysqli_query($conn, $checkCategory);
            if (mysqli_num_rows($resultCategory) > 0) {
                $duplicates[] = "Truss Ceiling Load";
            }
            ...
            if(!empty($duplicates)){
                ...
                $insertQuery = "INSERT INTO truss_ceiling_load (truss_ceiling_load, description, added_date, added_by) VALUES ('$truss_ceiling_load', '$description', NOW(), '$userid')";
    ...
    if ($action == "change_status") {
        ...
        $statusQuery = "UPDATE truss_ceiling_load SET status = '$new_status' WHERE truss_ceiling_load_id = '$truss_ceiling_load_id'";
    ...
    if ($action == 'hide_truss_ceiling_load') {
        $truss_ceiling_load_id = mysqli_real_escape_string($conn, $_POST['truss_ceiling_load_id']);
        $query = "UPDATE truss_ceiling_load SET hidden='1' WHERE truss_ceiling_load_id='$truss_ceiling_load_id'";
        ...
        ?>
            <div class="row pt-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="truss_ceiling_load" name="truss_ceiling_load" class="form-control"  value="<?= $truss_ceiling_load ?>"/>
                </div>
                </div>
            </div>
            ...
            <input type="hidden" id="truss_ceiling_load_id" name="truss_ceiling_load_id" class="form-control"  value="<?= $id ?>"/>
        <?php
    }
