...
$page_title = "Product Color Width";
?>
<style>
  ...
  <div class="row">
      ...
      <div class="col-9">
        ...
        <div class="datatables">
          <div class="card">
            <div class="card-body">
                <h4 class="card-title d-flex justify-content-between align-items-center"><?= $page_title ?>s List</h4>
              <div class="table-responsive">
                <table id="display_product_color_width" class="table table-striped table-bordered text-nowrap align-middle">
                  <thead>
                    <tr>
                      <th>Width</th>
                      <th>Description</th>
                  ...
                  <?php
                  $no = 1;
                  $query_product_color_width = "SELECT * FROM product_color_width WHERE hidden = 0";
                  $result_product_color_width = mysqli_query($conn, $query_product_color_width);            
                  while ($row_product_color_width = mysqli_fetch_array($result_product_color_width)) {
                      $product_color_width_id = $row_product_color_width['product_color_width_id'];
                      $product_color_width = $row_product_color_width['product_color_width'];
                      $description = $row_product_color_width['description'];
                      $db_status = $row_product_color_width['status'];

                      $date = new DateTime($row_product_color_width['last_edit']);
                      $last_edit = $date->format('m-d-Y');

                      $added_by = $row_product_color_width['added_by'];
                      $edited_by = $row_product_color_width['edited_by'];
                      ...
                      if ($row_product_color_width['status'] == '0') {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$product_color_width_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-danger bg-danger text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Inactive</div></a>";
                      } else {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$product_color_width_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Active</div></a>";
                      }
                  ?>
                  <tr id="product-row-<?= $no ?>">
                      <td><span class="product<?= $no ?> <?php if ($row_product_color_width['status'] == '0') { echo 'emphasize-strike'; } ?>"><?= $product_color_width ?></span></td>
                      <td class="notes" style="width:30%;"><?= $description ?></td>
                      <td class="last-edit" style="width:30%;">Last Edited <?= $last_edit ?> by  <?= $last_user_name ?></td>
                      <td><?= $status ?></td>
                      <td class="text-center" id="action-button-<?= $no ?>">
                          <?php if ($row_product_color_width['status'] == '0') { ?>
                                <a href="#" class="py-1 text-dark hideProductColorWidth" title="Archive" data-id="<?= $product_color_width_id ?>" data-row="<?= $no ?>">
                                    <i class="text-danger ti ti-trash fs-7"></i>
                                </a>
                          <?php } else { ?>
                                <a href="#" id="addModalBtn" title="Edit" class="d-flex align-items-center justify-content-center text-decoration-none" data-id="<?= $product_color_width_id ?>" data-type="edit">
                  ...
                  $(document).ready(function() {
                      ...
                      $(document).on('click', '.changeStatus', function(event) {
                          ...
                          var product_color_width_id = $(this).data('id');
                          var status = $(this).data('status');
                          var no = $(this).data('no');
                          $.ajax({
                              url: 'pages/product_color_width_ajax.php',
                              type: 'POST',
                              data: {
                                  product_color_width_id: product_color_width_id,
                                  status: status,
                              ...
                              success: function(response) {
                                  if (response == 'success') {
                                      if (status == 1) {
                                          ...
                                          $('#action-button-' + no).html('<a href="#" class="py-1 text-dark hideProductColorWidth" title="Archive" data-id="' + product_color_width_id + '" data-row="' + no + '"><i class="text-danger ti ti-trash fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        } else {
                                          $('#status-alert' + no).removeClass().addClass('alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0').text('Active');
                                          $(".changeStatus[data-no='" + no + "']").data('status', "1");
                                          $('.product' + no).removeClass('emphasize-strike'); // Remove emphasize-strike class
                                          $('#action-button-' + no).html('<a href="#" id="addModalBtn" title="Edit" data-id="' + product_color_width_id + '" data-type="edit" class="py-1"><i class="ti ti-pencil fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        }
                                  } else {
                      ...
                      $(document).on('click', '.hideProductColorWidth', function(event) {
                          event.preventDefault();
                          var product_color_width_id = $(this).data('id');
                          ...
                          $.ajax({
                              ...
                              data: {
                                  product_color_width_id: product_color_width_id,
                                  action: 'hide_product_color_width'
                              },
                              success: function(response) {
                                  if (response == 'success') {
                                      $('#product-row-' + rowId).remove(); // Remove the row from the DOM
                                  } else {
                                      alert('Failed to hide product color width.');
                                  }
                              },
                              error: function(jqXHR, textStatus, errorThrown) {
                                  alert('Error: ' + textStatus + ' - ' + errorThrown);
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form id="productColorWidthForm" class="form-horizontal">
                <div class="modal-body">
                    <div class="card">
                        <div class="card-body">
...
<script>
  $(document).ready(function() {
    ...
    var table = $('#display_product_color_width').DataTable({
        pageLength: 100
    });

    $('#display_product_color_width_filter').hide();
    ...
    $('#productColorWidthForm').on('submit', function(event) {
        event.preventDefault(); 
        ...
        $.ajax({
            ...
            success: function(response) {
              ...
              if (response.trim() === "success_update") {
                  ...
                  $('#responseMsg').text('Product color width updated successfully.');
                  $('#responseHeaderContainer').removeClass("bg-danger");
                  $('#responseHeaderContainer').addClass("bg-success");
                  $('#response-modal').modal("show");

                  $('#response-modal').on('hide.bs.modal', function () {
                    window.location.href = "?page=product_color_width";
                  });
              } else if (response === "success_add") {
                  $('#responseHeader').text("Success");
                  $('#responseMsg').text('New product color width added successfully.');
                  $('#responseHeaderContainer').removeClass("bg-danger");
                  $('#responseHeaderContainer').addClass("bg-success");
                  $('#response-modal').modal("show");
    ...
    $(document).on('click', '#downloadBtn', function(event) {
        window.location.href = "pages/product_color_width_ajax.php?action=download_excel";
    });
