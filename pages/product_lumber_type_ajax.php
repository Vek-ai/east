...
$table = 'product_lumber_type';
$test_table = 'product_lumber_type_excel';

$includedColumns = [
    'product_lumber_type_id'   => 'ID',
    'product_lumber_type'      => 'Lumber Type',
...
if(isset($_REQUEST['action'])) {
    ...
    if ($action == "add_update") {
        $product_lumber_type_id = mysqli_real_escape_string($conn, $_POST['product_lumber_type_id']);
        $product_lumber_type = mysqli_real_escape_string($conn, $_POST['product_lumber_type']);
        ...
        $checkQuery = "SELECT type_abreviations FROM product_lumber_type WHERE product_lumber_type_id = '$product_lumber_type_id'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $updateQuery = "UPDATE product_lumber_type 
                            SET product_lumber_type = '$product_lumber_type',
                                ...
                                last_edit = NOW(),
                                edited_by = '$userid'
                            WHERE product_lumber_type_id = '$product_lumber_type_id'";

            if (mysqli_query($conn, $updateQuery)) {
                echo "update-success";
                if ($old_abbr !== $type_abreviations) {
                    regenerateABR('product_lumber_type', $product_lumber_type_id);
                }
            } else {
                echo "Error updating product type: " . mysqli_error($conn);
            }
        } else {
            $insertQuery = "INSERT INTO product_lumber_type 
                            (product_lumber_type, type_abreviations, notes, dimensions, packs, added_date, added_by) 
                            VALUES 
                            ('$product_lumber_type', '$type_abreviations', '$notes', '$dimensions_sql', '$packs_sql', NOW(), '$userid')";
    ...
    if ($action == "change_status") {
        ...
        $statusQuery = "UPDATE product_lumber_type SET status = '$new_status' WHERE product_lumber_type_id = '$product_lumber_type_id'";
    ...
    if ($action == 'hide_product_lumber_type') {
        $product_lumber_type_id = mysqli_real_escape_string($conn, $_POST['product_lumber_type_id']);
        $query = "UPDATE product_lumber_type SET hidden='1' WHERE product_lumber_type_id='$product_lumber_type_id'";
    ...
    if ($action == 'fetch_modal_content') {
        $product_lumber_type_id = mysqli_real_escape_string($conn, $_POST['id']);
        ...
        ?>
            <div class="row pt-3">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" id="product_lumber_type" name="product_lumber_type" class="form-control"  value="<?= $row['product_lumber_type'] ?? '' ?>"/>
                    </div>
                </div>
            ...
            <div class="row pt-3">
                ...
                <div class="col-md-6">
                    <div class="d-flex justify-content-between align-items-center">
                        <label class="form-label">Lumber Length(s)</label>
            ...
            <input type="hidden" id="product_lumber_type_id" name="product_lumber_type_id" class="form-control"  value="<?= $product_lumber_type_id ?>"/>
        <?php
    }
    ...
    if ($action === 'fetch_table') {
        ...
        $query = "SELECT * FROM product_lumber_type WHERE hidden = 0";
        $result = mysqli_query($conn, $query);
    
        $data = [];
        while ($row = mysqli_fetch_assoc($result)) {
            $no = $row['product_lumber_type_id'];
            $product_lumber_type = $row['product_lumber_type'];
            ...
            $data[] = [
                'product_lumber_type' => $product_lumber_type,
                'type_abreviations' => $type_abreviations,
                'notes' => $notes,
