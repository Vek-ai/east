...
if(!empty($_REQUEST['truss_spacing_id'])){
  $truss_spacing_id = $_REQUEST['truss_spacing_id'];
  $query = "SELECT * FROM truss_spacing WHERE truss_spacing_id = '$truss_spacing_id'";
  $result = mysqli_query($conn, $query);            
  while ($row = mysqli_fetch_array($result)) {
      $truss_spacing_id = $row['truss_spacing_id'];
      $truss_spacing = $row['truss_spacing'];
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="table-responsive">
                <table id="display_truss_spacing" class="table table-striped table-bordered text-nowrap align-middle">
                  <thead>
                    <tr>
                      <th><?= $page_title ?></th>
                      <th>Description</th>
                  ...
                  <?php
                  $no = 1;
                  $query_truss_spacing = "SELECT * FROM truss_spacing WHERE hidden = 0";
                  $result_truss_spacing = mysqli_query($conn, $query_truss_spacing);            
                  while ($row_truss_spacing = mysqli_fetch_array($result_truss_spacing)) {
                      $truss_spacing_id = $row_truss_spacing['truss_spacing_id'];
                      $truss_spacing = $row_truss_spacing['truss_spacing'];
                      $unit_of_measure = $row_truss_spacing['unit_of_measure'];
                      $truss_spacing = $row_truss_spacing['truss_spacing'];
                      $description = $row_truss_spacing['description'];
                      $db_status = $row_truss_spacing['status'];

                      $date = new DateTime($row_truss_spacing['last_edit']);
                      $last_edit = $date->format('m-d-Y');

                      $added_by = $row_truss_spacing['added_by'];
                      $edited_by = $row_truss_spacing['edited_by'];
                      ...
                      if ($row_truss_spacing['status'] == '0') {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$truss_spacing_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-danger bg-danger text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Inactive</div></a>";
                      } else {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$truss_spacing_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Active</div></a>";
                      }
                  ?>
                  <tr id="product-row-<?= $no ?>">
                      <td><span class="product<?= $no ?> <?php if ($row_truss_spacing['status'] == '0') { echo 'emphasize-strike'; } ?>"><?= $truss_spacing .' ' .$unit_of_measure ?></span></td>
                  ...
                  $(document).ready(function() {
                      ...
                      $(document).on('click', '.changeStatus', function(event) {
                          ...
                          var truss_spacing_id = $(this).data('id');
                          var status = $(this).data('status');
                          var no = $(this).data('no');
                          $.ajax({
                              url: 'pages/truss_spacing_ajax.php',
                              type: 'POST',
                              data: {
                                  truss_spacing_id: truss_spacing_id,
                                  status: status,
                      ...
                      $(document).on('click', '.hideTrussSpacing', function(event) {
                          event.preventDefault();
                          var truss_spacing_id = $(this).data('id');
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form id="trussSpacingForm" class="form-horizontal">
                <div class="modal-body">
                    <div class="card">
                        <div class="card-body">
...
<script>
  $(document).ready(function() {
    ...
    var table = $('#display_truss_spacing').DataTable({
        pageLength: 100
    });

    $('#display_truss_spacing_filter').hide();
    ...
    $('#trussSpacingForm').on('submit', function(event) {
        event.preventDefault(); 
            ...
            success: function(response) {
              ...
              if (response.trim() === "success_update") {
                  ...
                  $('#response-modal').on('hide.bs.modal', function () {
                    window.location.href = "?page=truss_spacing";
                  });
    ...
    $(document).on('click', '#downloadBtn', function(event) {
        window.location.href = "pages/truss_spacing_ajax.php?action=download_excel";
    });
