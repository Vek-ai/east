...
<tr id="product-row-<?= $no ?>">
    ...
    <td class="text-center" id="action-button-<?= $no ?>">
        <?php if ($row_product_gauge['status'] == '0') { ?>
            <a href="#" class="btn btn-light py-1 text-dark hideProductGauge" data-id="<?= $product_gauge_id ?>" data-row="<?= $no ?>" style='border-radius: 10%;'>Archive</a>
        <?php } else { ?>
            <a href="/?page=product_gauge&product_gauge_id=<?= $product_gauge_id ?>" class="btn btn-primary py-1" style='border-radius: 10%;'>Edit</a>
        <?php } ?>
    </td>
</tr>
<?php
            ...
            success: function(response) {
                if (response == 'success') {
                    if (status == 1) {
                        ...
                        $('#action-button-' + no).html('<a href="#" class="btn btn-light py-1 text-dark hideProductGauge" data-id="' + product_gauge_id + '" data-row="' + no + '" style="border-radius: 10%;">Archive</a>');
                        $('#toggleActive').trigger('change');
                      } else {
                        $('#status-alert' + no).removeClass().addClass('alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0').text('Active');
                        $(".changeStatus[data-no='" + no + "']").data('status', "1");
                        $('.product' + no).removeClass('emphasize-strike'); // Remove emphasize-strike class
                        $('#action-button-' + no).html('<a href="/?page=product_gauge&product_gauge_id=' + product_gauge_id + '" class="btn btn-primary py-1" style="border-radius: 10%;">Edit</a>');
                        $('#toggleActive').trigger('change');
                      }
                } else {
