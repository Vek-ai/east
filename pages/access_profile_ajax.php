...
$test_table = 'access_profile_excel';
...
if(isset($_REQUEST['action'])) {
    ...
    if ($action == "add_update") {
        ...
        $access_profile = mysqli_real_escape_string($conn, $_POST['access_profile']);
        $description = mysqli_real_escape_string($conn, $_POST['description']);
        $userid = mysqli_real_escape_string($conn, $_POST['userid']);

        // SQL query to check if the record exists
        $checkQuery = "SELECT * FROM access_profile WHERE access_profile_id = '$access_profile_id'";
        $result = mysqli_query($conn, $checkQuery);

        if (mysqli_num_rows($result) > 0) {
            $updateQuery = "UPDATE access_profile SET access_profile = '$access_profile', description = '$description', last_edit = NOW(), edited_by = '$userid'  WHERE access_profile_id = '$access_profile_id'";
            if (mysqli_query($conn, $updateQuery)) {
                echo "success_update";
            } else {
                echo "Error updating Access Profile: " . mysqli_error($conn);
            }
        } else {
            $insertQuery = "INSERT INTO access_profile (access_profile, description, added_date, added_by) VALUES ('$access_profile', '$description', NOW(), '$userid')";
            if (mysqli_query($conn, $insertQuery)) {
                echo "sucess_add";
            } else {
                echo "Error adding Access Profile: " . mysqli_error($conn);
            }
        }
    } 
    ...
    if ($action == "change_status") {
        ...
        $statusQuery = "UPDATE access_profile SET status = '$new_status' WHERE access_profile_id = '$access_profile_id'";
    ...
    if ($action == 'hide_access_profile') {
        $access_profile_id = mysqli_real_escape_string($conn, $_POST['access_profile_id']);
        $query = "UPDATE access_profile SET hidden='1' WHERE access_profile_id='$access_profile_id'";
    ...
    if ($action == 'fetch_modal_content') {
        ...
        ?>
            <div class="row pt-3">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" id="access_profile" name="access_profile" class="form-control"  value="<?= $access_profile ?>"/>
                    </div>
                </div>
            </div>
            <div class="mb-3">
            ...
            <input type="hidden" id="access_profile_id" name="access_profile_id" class="form-control"  value="<?= $id ?>"/>
        <?php
    }
