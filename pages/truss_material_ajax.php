...
$table = 'truss_material';
$test_table = 'truss_material_excel';
...
if(isset($_REQUEST['action'])) {
    ...
    if ($action == "add_update") {
        $truss_material_id = mysqli_real_escape_string($conn, $_POST['truss_material_id']);
        ...
        $checkQuery = "SELECT * FROM truss_material WHERE truss_material_id = '$truss_material_id'";
        $result = mysqli_query($conn, $checkQuery);

        if (mysqli_num_rows($result) > 0) {
            // Record exists, fetch current values
            $row = mysqli_fetch_assoc($result);
            $current_truss_material = $row['truss_material'];

            $duplicates = array();

            // Check for duplicates only if the new values are different from the current values
            if ($truss_material != $current_truss_material) {
                $checkCategory = "SELECT * FROM truss_material WHERE truss_material = '$current_truss_material'";
                $resultCategory = mysqli_query($conn, $checkCategory);
                if (mysqli_num_rows($resultCategory) > 0) {
                    $duplicates[] = "Truss Material";
                }
            }
            ...
            if (!empty($duplicates)) {
                ...
                $updateQuery = "UPDATE truss_material SET truss_material = '$truss_material', description = '$description', last_edit = NOW(), edited_by = '$userid'  WHERE truss_material_id = '$truss_material_id'";
                if (mysqli_query($conn, $updateQuery)) {
                    echo "success_update";
                } else {
                    echo "Error updating Truss Material: " . mysqli_error($conn);
                }
            }
        } else {
            // Record does not exist, perform duplicate checks before inserting
            $duplicates = array();
            $checkCategory = "SELECT * FROM truss_material WHERE truss_material = '$truss_material'";
            $resultCategory = mysqli_query($conn, $checkCategory);
            if (mysqli_num_rows($resultCategory) > 0) {
                $duplicates[] = "Truss Material";
            }
            ...
            if(!empty($duplicates)){
                ...
                $insertQuery = "INSERT INTO truss_material (truss_material, description, added_date, added_by) VALUES ('$truss_material', '$description', NOW(), '$userid')";
                if (mysqli_query($conn, $insertQuery)) {
                    echo "sucess_add";
                } else {
                    echo "Error adding Truss Material: " . mysqli_error($conn);
                }
            }
        }
    } 
    ...
    if ($action == "change_status") {
        ...
        $statusQuery = "UPDATE truss_material SET status = '$new_status' WHERE truss_material_id = '$truss_material_id'";
    ...
    if ($action == 'hide_truss_material') {
        $truss_material_id = mysqli_real_escape_string($conn, $_POST['truss_material_id']);
        $query = "UPDATE truss_material SET hidden='1' WHERE truss_material_id='$truss_material_id'";
        ...
        ?>
            <div class="row pt-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="truss_material" name="truss_material" class="form-control"  value="<?= $truss_material ?>"/>
                </div>
                </div>
            </div>
            ...
            <input type="hidden" id="truss_material_id" name="truss_material_id" class="form-control"  value="<?= $id ?>"/>
        <?php
    }
