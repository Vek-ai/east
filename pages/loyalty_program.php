...
<script>
  $(document).ready(function() {
    ...
    $('#display_loyalty_program_filter').hide();
            ...
            success: function(response) {
                if (response == 'success') {
                    if (status == 1) {
                        ...
                        $('#action-button-' + no).html('<a href="#" title="Archive" class="py-1 text-dark hideLoyaltyProgram" data-id="' + loyalty_id + '" data-row="' + no + '"><i class="text-danger ti ti-trash fs-7"></i></a>');
                        $('#toggleActive').trigger('change');
                      } else {
                        $('#status-alert' + no).removeClass().addClass('alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0').text('Active');
                        $(".changeStatus[data-no='" + no + "']").data('status', "1");
                        $('.product' + no).removeClass('emphasize-strike');
                        $('#action-button-' + no).html('<a href="#" id="addModalBtn" title="Edit" class="d-flex align-items-center justify-content-center text-decoration-none" data-id="' + loyalty_id + '" data-type="edit"><i class="ti ti-pencil fs-7"></i></a>');
                        $('#toggleActive').trigger('change');
                      }
                } else {
    ...
    $(document).on('click', '#downloadBtn', function(event) {
        window.location.href = "pages/loyalty_program_ajax.php?action=download_excel";
    });
