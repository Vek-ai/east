            ...
            <div class="card">
              <div class="card-body">
                ...
                <div class="table-responsive">
                  <table id="display_lumber_type" class="table table-striped table-bordered align-middle">
                    <thead>
                        <tr>
                            <th><?= $page_title ?></th>
                            <th>Abbreviation</th>
...
<script>
  $(document).ready(function() {
    ...
    var columns = [];
    columns.push({ data: 'product_lumber_type' });
    ...
    var table = $('#display_lumber_type').DataTable({
        pageLength: 100,
        ajax: {
            url: 'pages/product_lumber_type_ajax.php',
            type: 'POST',
    ...
    $('#display_lumber_type_filter').hide();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response == 'success') {
                    ...
                    alert('Failed to hide Lumber Type.');
                }
            },
            error: function(jqXHR, textStatus, errorThrown) {
                alert('Error: ' + textStatus + ' - ' + errorThrown);
        ...
        $.ajax({
            ...
            success: function(response) {
              ...
              if (response.trim() === "update-success") {
                  ...
                  $('#responseMsg').text("Lumber Type updated successfully.");
                  ...
                  $('#responseHeader').text("Success");
                  $('#responseMsg').text("New Lumber Type added successfully.");
                  $('#responseHeaderContainer').removeClass("bg-danger");
                  $('#responseHeaderContainer').addClass("bg-success");
                  $('#response-modal').modal("show");
                  table.ajax.reload(null, false);
    ...
    $(document).on('click', '#downloadBtn', function(event) {
        window.location.href = "pages/product_lumber_type_ajax.php?action=download_excel";
    });
    ...
    $(document).on('click', '#addModalBtn', function(event) {
        ...
        if(type == 'edit'){
          $('#add-header').html('Update Lumber Type');
        }else{
          $('#add-header').html('Add Lumber Type');
        }
