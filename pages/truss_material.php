...
if(!empty($_REQUEST['truss_material_id'])){
  $truss_material_id = $_REQUEST['truss_material_id'];
  $query = "SELECT * FROM truss_material WHERE truss_material_id = '$truss_material_id'";
  $result = mysqli_query($conn, $query);            
  while ($row = mysqli_fetch_array($result)) {
      $truss_material_id = $row['truss_material_id'];
      $truss_material = $row['truss_material'];
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="table-responsive">
                <table id="display_truss_material" class="table table-striped table-bordered text-nowrap align-middle">
                  <thead>
                    <tr>
                      <th><?= $page_title ?></th>
                      <th>Description</th>
                  ...
                  <?php
                  $no = 1;
                  $query_truss_material = "SELECT * FROM truss_material WHERE hidden = 0";
                  $result_truss_material = mysqli_query($conn, $query_truss_material);            
                  while ($row_truss_material = mysqli_fetch_array($result_truss_material)) {
                      $truss_material_id = $row_truss_material['truss_material_id'];
                      $truss_material = $row_truss_material['truss_material'];
                      $description = $row_truss_material['description'];
                      $db_status = $row_truss_material['status'];

                      $date = new DateTime($row_truss_material['last_edit']);
                      $last_edit = $date->format('m-d-Y');

                      $added_by = $row_truss_material['added_by'];
                      $edited_by = $row_truss_material['edited_by'];
                      ...
                      if ($row_truss_material['status'] == '0') {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$truss_material_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-danger bg-danger text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Inactive</div></a>";
                      } else {
                          $status = "<a href='#' class='changeStatus' data-no='$no' data-id='$truss_material_id' data-status='$db_status'><div id='status-alert$no' class='alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0' style='border-radius: 5%;' role='alert'>Active</div></a>";
                      }
                  ?>
                  <tr id="product-row-<?= $no ?>">
                      <td><span class="product<?= $no ?> <?php if ($row_truss_material['status'] == '0') { echo 'emphasize-strike'; } ?>"><?= $truss_material ?></span></td>
                      <td class="notes" style="width:30%;"><?= $description ?></td>
                      <td class="last-edit" style="width:30%;">Last Edited <?= $last_edit ?> by  <?= $last_user_name ?></td>
                      <td><?= $status ?></td>
                      <td class="text-center" id="action-button-<?= $no ?>">
                          <?php if ($row_truss_material['status'] == '0') { ?>
                                <a href="#" title="Archive" class="py-1 text-dark hideTrussMaterial" data-id="<?= $truss_material_id ?>" data-row="<?= $no ?>">
                                    <i class="text-danger ti ti-trash fs-7"></i>
                                </a>
                          <?php } else { ?>
                                <a href="#" title="Edit" id="addModalBtn" class="d-flex align-items-center justify-content-center text-decoration-none" data-id="<?= $truss_material_id ?>" data-type="edit">
                  ...
                  $(document).ready(function() {
                      ...
                      $(document).on('click', '.changeStatus', function(event) {
                          ...
                          var truss_material_id = $(this).data('id');
                          var status = $(this).data('status');
                          var no = $(this).data('no');
                          $.ajax({
                              url: 'pages/truss_material_ajax.php',
                              type: 'POST',
                              data: {
                                  truss_material_id: truss_material_id,
                                  status: status,
                              ...
                              success: function(response) {
                                  if (response == 'success') {
                                      if (status == 1) {
                                          ...
                                          $('#action-button-' + no).html('<a href="#" title="Archive" class="py-1 text-dark hideTrussMaterial" data-id="' + truss_material_id + '" data-row="' + no + '"><i class="text-danger ti ti-trash fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        } else {
                                          $('#status-alert' + no).removeClass().addClass('alert alert-success bg-success text-white border-0 text-center py-1 px-2 my-0').text('Active');
                                          $(".changeStatus[data-no='" + no + "']").data('status', "1");
                                          $('.product' + no).removeClass('emphasize-strike'); // Remove emphasize-strike class
                                          $('#action-button-' + no).html('<a href="#" title="Edit" id="addModalBtn" data-id="' + truss_material_id + '" data-type="edit" class="py-1"><i class="ti ti-pencil fs-7"></i></a>');
                                          $('#toggleActive').trigger('change');
                                        }
                                  } else {
                      ...
                      $(document).on('click', '.hideTrussMaterial', function(event) {
                          event.preventDefault();
                          var truss_material_id = $(this).data('id');
...
<div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form id="trussMaterialForm" class="form-horizontal">
                <div class="modal-body">
                    <div class="card">
                        <div class="card-body">
...
<script>
  $(document).ready(function() {
    ...
    var table = $('#display_truss_material').DataTable({
        pageLength: 100
    });

    $('#display_truss_material_filter').hide();
    ...
    $('#trussMaterialForm').on('submit', function(event) {
        event.preventDefault(); 
            ...
            success: function(response) {
              ...
              if (response.trim() === "success_update") {
                  ...
                  $('#response-modal').on('hide.bs.modal', function () {
                    window.location.href = "?page=truss_material";
                  });
    ...
    $(document).on('click', '#downloadBtn', function(event) {
        window.location.href = "pages/truss_material_ajax.php?action=download_excel";
    });
